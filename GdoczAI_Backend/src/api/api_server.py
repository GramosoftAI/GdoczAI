...
def get_current_user(
    ...
    """Verify JWT token or X-API-Key header and get current user"""
    if not api_auth_manager:
        raise HTTPException(status_code=503, detail="Authentication service not initialized")
    
    # 1. Try JWT validation first if credentials provided
    if credentials:
    ...
    # 2. Try API key validation if API key provided
    if x_api_key:
    ...
    # 3. Raise 401 if neither is valid/present
    raise HTTPException(
        status_code=401, 
        detail="Not authenticated. Valid Authorization token or X-API-Key is required."
    )
