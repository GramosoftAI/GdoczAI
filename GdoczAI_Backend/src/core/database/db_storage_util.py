...
class DatabaseStorage:
                ...
                if existing_record:
                    ...
                    cursor.execute(
                        ...
                        (file_name, page_count, processed_on, processing_duration,
                         json_output, markdown_output, model, missed_keys,
                         token_usage, error_details,
    ...
    def get_markdown_by_request_id(self, request_id: str) -> Optional[str]:
        """
        Fetch the markdown_output column from processed_files for a given request_id.

        Args:
            request_id: The request ID to look up.

        Returns:
            markdown_output string if found and non-empty, else None.
        """
        with self.lock:
            conn = None
            try:
                ...
                cursor.execute(
                    "SELECT markdown_output FROM processed_files WHERE request_id = %s LIMIT 1",
                    (request_id,)
                )
                row = cursor.fetchone()
                ...
                if row is None:
                    logger.warning(
                        f"?? [DB] No record found for request_id={request_id}"
                    )
                    return None

                markdown_output = row[0]
                if not markdown_output or not markdown_output.strip():
                    logger.warning(
                        f"?? [DB] markdown_output is empty for request_id={request_id}"
                    )
                    return ""

                logger.info(
                    f"? [DB] Fetched markdown_output ({len(markdown_output)} chars) "
                ...
                logger.error(
                    f"? [DB] Database error fetching markdown for "
