...
class Config:
    ...
    def __init__(self):
        ...
        # ----------------------------------------------------------------
        self.mistral_ocr_api_key = self.config.get('mistral_ocr', {}).get('api_key')
        self.mistral_ocr_model   = self.config.get('mistral_ocr', {}).get('model')
        self.mistral_ocr_timeout = self.config.get('mistral_ocr', {}).get('timeout', 300)

        if self.mistral_ocr_api_key:
            logger.info(f"? MISTRAL OCR: ENABLED")
            logger.info(f"  Model  : {self.mistral_ocr_model}")
            logger.info(f"  Timeout: {self.mistral_ocr_timeout}s")
        else:
            logger.error(f"? MISTRAL OCR: DISABLED (No API key)")
            logger.error("   OCR Server requires Mistral OCR to be configured!")
...
try:
    ...
    logger.error("? Google GenAI not available. JSON generation will fail!")
...
# ============================================================================
# MISTRAL OCR AVAILABILITY CHECK
# ============================================================================
MISTRAL_OCR_AVAILABLE = False
if config.mistral_ocr_api_key:
    MISTRAL_OCR_AVAILABLE = True
    logger.info("? MISTRAL OCR available (API configured)")
else:
    logger.error("? MISTRAL OCR not available - No API key")
    logger.error("   OCR Server cannot function without Mistral OCR!")
...
logger.info("=" * 60)
logger.info(f"? MISTRAL OCR: {'AVAILABLE' if MISTRAL_OCR_AVAILABLE else 'NOT AVAILABLE'}")
...
if not MISTRAL_OCR_AVAILABLE:
    ...
    logger.error("=" * 60)
    logger.error("Mistral OCR is NOT configured!")
    logger.error("OCR Server cannot function without Mistral OCR.")
    logger.error("")
    logger.error("To fix:")
    logger.error("  1. Set mistral_ocr.api_key in config.yaml or environment")
...
if not GEMINI_AVAILABLE:
    ...
    logger.error("OCR Server cannot function without Gemini.")
    logger.error("")
    logger.error("To fix:")
...
if (
    ...
    logger.info("  ? Mistral OCR for PDF -> Markdown conversion")
