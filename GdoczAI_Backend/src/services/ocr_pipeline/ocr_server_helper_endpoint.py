...
async def ocr_markdown_only(
    ...
    from src.services.ocr_pipeline.ocr_server_app import (
        storage_manager, mistral_processor, qwen_processor, chandra_processor,
            ...
            return JSONResponse(
                ...
                content={
                    ...
                    "error": (
                        ...
                        "Supported models: mistral, qwen, chandra"
                    ),
                    "request_id": request_id
                }
            )
...
async def extract_markdown(
    ...
    # ============================================================
    # INPUT MODE RESOLUTION: markdown_content OR request_id
    # ============================================================

    # Mode 2: request_id provided and markdown_content not given ? fetch from DB
    if not request.markdown_content:
        if not request.request_id:
            ...
            return JSONResponse(
                ...
                content={
                    "success": False,
                    "error": "Either markdown_content or request_id must be provided"
                }
            )

        logger.info(
            f"?? [extract/markdown] No markdown_content provided. "
            f"Fetching from DB for request_id={request.request_id}"
        )

        fetched_markdown = db_storage.get_markdown_by_request_id(request.request_id)

        if fetched_markdown is None:
            # request_id not found in DB
            logger.warning(
                f"? [extract/markdown] request_id not found in DB: {request.request_id}"
            )
            return JSONResponse(
                status_code=404,
                content={
                    "success": False,
                    "error": f"No record found for request_id: {request.request_id}"
                }
            )

        if not fetched_markdown.strip():
            # Row found but markdown_output is empty/null
            logger.warning(
                f"? [extract/markdown] markdown_output is empty for request_id={request.request_id}"
            )
            return JSONResponse(
                status_code=400,
                content={
                    "success": False,
                    "error": f"markdown_output is empty for request_id: {request.request_id}"
                }
            )

        logger.info(
            f"? [extract/markdown] Markdown fetched from DB "
            f"({len(fetched_markdown)} chars) for request_id={request.request_id}"
        )
        request.markdown_content = fetched_markdown

    # Mode 1: markdown_content provided directly (existing path  fall through)
