...
class MarkdownExtractionRequest(BaseModel):
    ...
    markdown_content: Optional[str] = Field(None, description="Markdown content to extract from (optional if request_id is provided)")
    request_id: Optional[str] = Field(None, description="Request ID to fetch markdown from DB (used when markdown_content is not provided)")
