"""
Database connection module for smtp_connector queries.
Mirrors document_pipeline_database.py exactly.
"""
...
def query_active_smtp_connectors() -> List[Dict[str, Any]]:
    ...
    try:
        ...
        logger.error(f"Unexpected error querying smtp_connector: {e}")
        raise
    finally:
        if conn:
            _db_storage._put_connection(conn)

def get_smtp_connector_by_id(connector_id: int) -> Dict[str, Any]:
...
def get_smtp_connectors_by_user_id(user_id: int) -> List[Dict[str, Any]]:
