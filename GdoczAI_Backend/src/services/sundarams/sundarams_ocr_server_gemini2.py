...
class GeminiHeavyMethods:
    ...
    def _validate_json_response(self, response_text: str) -> Dict:
        try:
            ...
            # Auto-flatten "fields" list structure if present
            if isinstance(parsed_json, dict) and "fields" in parsed_json and isinstance(parsed_json["fields"], list):
                logger.info("?? Found 'fields' list format in JSON response - flattening...")
                flat_json = {}
                for field in parsed_json["fields"]:
                    if isinstance(field, dict) and "field_name" in field:
                        flat_json[field["field_name"]] = field.get("value")
                # Preserve any other root-level keys (like status, expense, line_items)
                for k, v in parsed_json.items():
                    if k != "fields":
                        flat_json[k] = v
                parsed_json = flat_json
    ...
    async def _process_chunks_with_gemini(self, chunks: List, chunk_count: int,
        ...
        """Process all chunks with Gemini."""
        logger.info("?? Using Gemini for chunk processing")
        ...
        for i, (response_text, prompt_tokens, response_tokens) in enumerate(responses, 1):
            ...
            if chunk_json and chunk_json.get("status") != "error":
                ...
                logger.info(f"? Chunk {i} extracted successfully")
            else:
    ...
    async def generate_json_from_markdown_async(
        ...
        logger.info("=" * 80)
        logger.info(f"?? USING GEMINI MODEL: {self.model_name} (Max Tokens: {self.max_tokens})")
        logger.info("=" * 80)
            ...
            if self.config.chunking_enabled:
                ...
                if has_langchain_keys and len(langchain_keys) > 0:
                    ...
                    if disable_unstructured_chunking:
                        logger.info("=" * 80)
                        logger.info("?? MISTRAL MODE - NO LANGCHAIN KEYS - CHUNKING DISABLED")
                ...
                # ============================================================
                # MULTIPLE CHUNKS: ALWAYS GEMINI
                # ============================================================
                else:
                    logger.info("=" * 80)
                    logger.info(f"?? STEP 1: Processing {chunk_count} chunks in PARALLEL")
                    logger.info(f"?? Model: {self.model_name} (chunk-level)")
                    logger.info("=" * 80)
