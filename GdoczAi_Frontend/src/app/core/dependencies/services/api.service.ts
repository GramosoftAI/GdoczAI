...
export class ApiService {
    ...
    post(url: string, data: any, tost?: { type: 'NT' | 'CR' | 'ED' | 'DL' | 'CL' | 'UP' | 'SE' | 'OC' | 'OR' | 'OS' | 'LO', name?: string }, NoToStopLoader?: 'TRUE' | 'FALSE'): Observable<any> {
        const headers = new HttpHeaders().set('screen', tost ? tost.type : 'NT').set('NoToStopLoader', NoToStopLoader ? NoToStopLoader : 'FALSE');
        return this.http.post(`${url}`, data, { headers: headers });
    }

    patch(url: string, data: any, tost?: { type: 'NT' | 'CR' | 'ED' | 'DL' | 'CL' | 'UP' | 'SE', name?: string }): Observable<any> {
        const headers = new HttpHeaders().set('screen', tost ? tost.type : 'NT');
        return this.http.patch(`${url}`, data, { headers: headers });
    }

    put(url: string, data: any, tost?: { type: 'NT' | 'CR' | 'ED' | 'DL' | 'CL' | 'UP' | 'SE', name?: string }): Observable<any> {
        const headers = new HttpHeaders().set('screen', tost ? tost.type : 'NT');
