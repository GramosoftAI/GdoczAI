...
from src.services.ocr_pipeline.ocr_server_mistral_processor import MistralProcessor
from src.services.ocr_pipeline.ocr_server_qwen_processor    import QwenProcessor
from src.services.ocr_pipeline.ocr_server_chandra_processor import ChandraProcessor
...
app = FastAPI(
    ...
    description=(
        ...
        "Mistral OCR or Qwen3-VL (DeepInfra) or Chandra (Datalab Marker)"
    ),
    version="15.2.0",
    docs_url="/ocr/docs",
    redoc_url="/ocr/redoc",
...
async def startup_event():
    ...
    logger.info("[OK] Mistral OCR ready (no warmup required)")
    logger.info("     Mistral OCR uses API-based processing")
    ...
    logger.info(
        "[>>] OCR Backends: Mistral OCR (mistral-ocr) | "
    ...
    logger.info("     MISTRAL MODE:")
    logger.info("       Mistral OCR -> Strict Validation -> Conditional Split -> Smart Gemini")
    logger.info("       Fallback OCR    : DISABLED (Mistral only)")
    ...
    logger.info("       Validation fail : HTTP 400 error (no retry)")
    logger.info("=" * 60)
    logger.info("[>>] GEMINI MODEL SELECTION:")
    logger.info("     All Content Lengths     -> Gemini 2.5 Flash (65K output)")
...
async def _run_ocr_processing(
    ...
    # Compatible with MistralProcessor, QwenProcessor, and ChandraProcessor.
...
from src.services.ocr_pipeline.ocr_server_segment_endpoint import router as segment_router
        ...
        "data": [
            {
                ...
                "owned_by":    "mistralai",
                "description": (
                    "Mistral OCR (mistral-ocr-latest) - PDF/image extraction with strict "
...
if __name__ == "__main__":
    ...
    logger.info(
        "OCR Backends: Mistral OCR (mistral-ocr-latest) | "
        "Qwen (Qwen/Qwen3-VL-235B-A22B-Instruct) | "
        "Chandra (Datalab Marker API)"
    )
    logger.info("Processing Mode  : PDF/Images -> Mistral/Qwen/Chandra API")
    logger.info(
        "Chunking         : %s",
    ...
    logger.info("Supported models : mistral | qwen | chandra")
