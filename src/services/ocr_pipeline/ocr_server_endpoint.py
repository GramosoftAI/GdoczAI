...
async def ocr_pdf(
    ...
    from src.services.ocr_pipeline.ocr_server_app import (
        ...
        mistral_processor, qwen_processor, chandra_processor,
    ...
    try:
        ...
        with ocr_processing_lock:
            ...
            if model_lower == "standard": #sub name for mistral
                ...
                ocr_engine_label = "Mistral OCR (Mistral AI)"
                logger.info(f"?? Mistral OCR mode activated - proceeding with Mistral OCR processing (model arg: {model})")
            ...
            if db_storage:
                try:
                    ...
                    logger.info(
                        ...
                        f"{len(final_markdown) if final_markdown else 0} chars"
                    )
                    logger.info(f"   ?? Model Used    : {model_lower}")
                    ...
                    if stored:
                        ...
                        logger.info(f"   ? Model tracking     : {model_lower}")
