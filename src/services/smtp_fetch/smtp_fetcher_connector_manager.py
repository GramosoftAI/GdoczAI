...
class EmailConnectorManager:
    ...
    def _create_imap_config_from_row(self, connector_row: dict) -> IMAPConnectorConfig:
        ...
        imap_server = connector_row.get('imap_server') or ''
        imap_port = connector_row.get('imap_port') or 993

        if not imap_server:
            logger.warning(
                f"imap_server is empty for user_id={connector_row['user_id']}, "
                f"connector id={connector_row['id']}. Skipping connector."
            )
            raise ValueError(
                f"imap_server cannot be empty for connector id={connector_row['id']}"
            )
    ...
    def _start_connector(self, connector_row: dict):
        ...
        try:
            logger.info("=" * 80)
            logger.info("STARTING NEW CONNECTOR")
            logger.info(f"Connector ID   : {connector_id}")
            logger.info(f"User ID        : {user_id}")
            email_masked = (
                "***" + connector_row['email_id'][connector_row['email_id'].find('@'):]
                if '@' in connector_row['email_id']
                else connector_row['email_id']
            )
            logger.info(f"Email          : {email_masked}")
            logger.info(f"IMAP Server    : {connector_row.get('imap_server', 'N/A')}")
            logger.info(f"IMAP Port      : {connector_row.get('imap_port', 993)}")
            ...
            logger.error(f"Invalid connector config for connector {connector_id}: {e}")
        except Exception as e:
...
class RunningEmailConnector:
    ...
    def detect_credential_changes(self, new_connector_row: dict) -> dict:
        ...
        monitored_columns = [
            ...
            'imap_server', 'imap_port', 'interval_minute'
        ]
    ...
    def is_critical_change(self, changes: dict) -> bool:
        critical_fields = {'email_id', 'app_password', 'imap_server', 'imap_port'}
