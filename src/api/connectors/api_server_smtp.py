...
- SMTP email credentials, approved senders, IMAP server/port, and interval configuration
"""
...
def create_user_smtp_routes(app, get_current_user):
    ...
    async def create_user_smtp(
        ...
        imap_server: Optional[str] = Form(
            None, description="IMAP server hostname (e.g. imap.gmail.com)"
        ),
        imap_port: Optional[int] = Form(
            None, description="IMAP server port (e.g. 993)"
        ),
        ...
        try:
            ...
            cursor.execute(
                ...
                (user_id, email_id, app_password, approved_senders, imap_server, imap_port, interval_minute, is_active)
                VALUES (%s, %s, %s, %s, %s, %s, %s, %s)
                RETURNING id, user_id, email_id, app_password, approved_senders,
                          imap_server, imap_port, interval_minute, is_active, created_at, updated_at
            """,
                (
                    user_id,
                    email_id,
            ...
            logger.info(f"   [IMAP] IMAP Server: {imap_server or 'Not specified'}")
            logger.info(f"   [IMAP] IMAP Port: {imap_port or 'Not specified'}")
    ...
    async def update_user_smtp(
        ...
        try:
            ...
            if imap_server is not None:
                update_fields.append("imap_server = %s")
                update_values.append(imap_server)

            if imap_port is not None:
                update_fields.append("imap_port = %s")
                update_values.append(imap_port)
            ...
            if imap_server:
                logger.info(f"   [IMAP] New IMAP Server: {imap_server}")
            if imap_port:
                logger.info(f"   [IMAP] New IMAP Port: {imap_port}")
            if interval_minute:
